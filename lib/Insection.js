(function (root, factory) {
    ...
    Tree.prototype.augmentNode = function (node) {
        ...
        if (node.left || node.right) {
            node.value = this.augmenterfn(node.left.value, node.right.value);
        }
    };
    ...
    function augmenter(x, y) {
        x = x && x.interval;
        y = y && y.interval;
        if (!x) return y;
        if (!y) return x;
        return Insection.interval(
            x.isStartOpen() ? y.startString : x.startString,
            min(x.start, y.start),
            max(x.end, y.end),
            x.isEndOpen() ? y.endString : x.endString
        );
    }
