(function (root, factory) {
    ...
    function Tree(comparefn, augmenterfn) {
        this.root = null;
    ...
    Tree.prototype.augmentNode = function (node) {
    ...
    Tree.prototype.augmentAncestors = function (node) {
    ...
    Tree.prototype.augmentNodeAndAncestors = function (node) {
        if (node) {
            this.augmentNode(node);
    ...
    Tree.prototype.findNode = function (navigator) {
        var node = this.root;
        while (node !== null) {
    ...
    Tree.prototype.find = function (navigator) {
    ...
    function augmenter(value, x, y) {
        ...
        if (!x) return new ClosedClosedInterval(
            min(y.start, interval.start),
            max(y.end, interval.end)
        );

        if (!y) return new ClosedClosedInterval(
            min(x.start, interval.start),
            max(x.end, interval.end)
        );

        return new ClosedClosedInterval(
            min(min(x.start, y.start), interval.start),
            max(max(x.end, y.end), interval.end)
        );
    }
    ...
    function Insection(valueComparer) {
        ...
        this.data = new Tree(function (x, y) {
